...
<?php
    ...
    //テーブル作成//
    $pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
    $sql = "CREATE TABLE IF NOT EXISTS tbtest"//もしまだこのテーブルが存在しないなら//
    ." ("
    ...
    if(isset($_POST["number"],$_POST["name"],$_POST["message"],$_POST["password"])){
        ...
        if($number1=="" && $name1!="" && $message1!="" && $password1!=""){
            ...
            //データベースに投稿内容を書き込み//
            $sql = $pdo -> prepare("INSERT INTO tbtest (name,message,password,date) VALUES (:name,:message,:password,:date)");
        ...
        //編集の時//
            ...
            $password = $password1;
            $sql = 'UPDATE tbtest SET name=:name,message=:message,password=:password WHERE id=:id';
    ...
    if(isset($_POST["editNumber"],$_POST["editPassword"])){
        ...
        //データを取得する//
        $sql = 'SELECT * FROM tbtest';//WHERE id=:idなどと付け加えると条件を絞れる//
        $stmt = $pdo->query($sql);
...
<body>
    ...
    <form action="" method="post">
        ...
        <input type="password" name="password" placeholder="パスワード" value="<?php if(isset($editPass)){echo $editPass;} ?>">
    ...
    <h2>＜編集＞</h2>
    <form action="" method="post">
        <input type="number" name="editNumber" placeholder="編集する投稿番号">
